...
{
  while(1) {
      acquire(&rwlk->l); // Lock the struct
      
      // Writer Priority Rule:
      // Enter only if: No Active Writer AND No Waiting Writer
      if(rwlk->writer == 0 && rwlk->writer_waiting == 0) {
        rwlk->readers++;
        release(&rwlk->l);
        return; // Acquired!
      }
      
      release(&rwlk->l); // Failed, release and spin again
    }
}
...
write_acquire_inner(struct rwspinlock *rwlk)
{
  ...
  // 2. Spin until safe to write
  while(1) {
    acquire(&rwlk->l);
    
    // Can only write if: No Readers AND No Active Writer
    if(rwlk->readers == 0 && rwlk->writer == 0) {
      rwlk->writer = 1;       // I am now the active writer
      rwlk->writer_waiting--; // I am no longer waiting
      release(&rwlk->l);
      return; // Acquired!
    }
